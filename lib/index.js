...
var warnIfNotReduxAction = function warnIfNotReduxAction(config, key) {
  ...
  var isNotReduxAction = maybeAction === null || (typeof maybeAction === 'undefined' ? 'undefined' : _typeof(maybeAction)) !== 'object' || typeof maybeAction.type !== 'string' || maybeAction.type === '';

  if (isNotReduxAction && console.warn) {
    var msg = key + ' must be a proper redux action, ' + 'i.e. it must be an object and have a non-empty string type. ' + ('Instead you provided: ' + JSON.stringify(maybeAction, null, 2));
    console.warn(msg);
  }
};
